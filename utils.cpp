#include <glad/glad.h> // 确保先包含 Glad
#include <GLFW/glfw3.h> // 然后再包含 GLFW
#include "Utils.h"
using namespace std;
...
GLuint Utils::loadTexture(const char* texImagePath)
{
	...
	if (textureRef == 0) {
		std::cout << "didn't find texture file " << texImagePath << std::endl;
	}
	...
	glGetFloatv(GL_MAX_TEXTURE_MAX_ANISOTROPY, &anisoset); // 使用正确的常量
	glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAX_ANISOTROPY, anisoset); // 使用正确的常量
